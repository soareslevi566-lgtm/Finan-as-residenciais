...
public class ServiceTests : IDisposable
{
 ...
 [Fact] public async Task Exclui_transacao_existente(){var p=await Pessoa();var t=await transacoes.CriarAsync(new("Remover",15,TipoTransacao.Despesa,p.Id),default);await transacoes.ExcluirAsync(t.Id,default);Assert.Empty(await transacoes.ListarAsync(p.Id,null,null,null,null,null,default));}
 [Fact] public async Task Impede_excluir_transacao_inexistente()=>await Assert.ThrowsAsync<AppException>(()=>transacoes.ExcluirAsync(999,default));
}
