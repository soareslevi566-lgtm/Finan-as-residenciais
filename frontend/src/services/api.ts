...
export const transacoesApi={listar:(params?:FiltrosTransacao)=>api.get<Transacao[]>('/transacoes',{params}),criar:(d:{descricao:string;valor:number;tipo:TipoTransacao;pessoaId:number;categoria?:CategoriaTransacao;data?:string})=>api.post<Transacao>('/transacoes',d)};
